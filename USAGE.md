...
本文说明如何安装、初始化和日常使用 LLM Wiki Agent。`AGENTS.md` 是 agent 入口，`WIKI.md` 是完整工作规则。
...
- PowerShell，用于运行 `scripts/init.ps1`。
- Python，由 `uv` 管理虚拟环境。
...
1. 读取 `AGENTS.md`，确认任务属于 Add Knowledge。
2. 读取 `WIKI.md`，使用其中的 intake 和 source review 规则。
...
没有明确要求时，agent 应先列出建议回流的要点并等待确认。确认后，讨论来源记录写入 `reviews/reflection/YYYY-MM-DD.md`，相关 wiki 页面用 reflect callout 引用。

## 10. 维护 Wiki

当你要求 agent 维护 wiki 时，可以使用这些自然语言请求：
...
先运行初始化脚本。脚本会尝试安装 `uv`。如果安装失败，按错误提示手动安装 `uv`，然后重新运行：
...
公开分享本 agent 包时，不建议提交个人 `inbox/`、`raw/`、`intake/`、`wiki/`、`logs/`、`questions/` 或 `artifacts/` 内容。默认 `.gitignore` 会忽略这些本地运行目录。若你有意把个人 wiki 也纳入版本管理，需要自行调整 `.gitignore`。
