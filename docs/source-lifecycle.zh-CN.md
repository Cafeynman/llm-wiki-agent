...
每个 `source.md` 创建后，对该文件运行 source-extraction 的 intake file audit。正常文件不输出内容。若审计报告 `large_source`，并且来源中存在可靠语义边界，则在 Source Review Gate 前创建语义化 `chunks/`。创建 chunks 后，对临时 intake 文件夹运行 chunk audit；推进前先解决 hard errors，将 warnings 带入审查记录。
