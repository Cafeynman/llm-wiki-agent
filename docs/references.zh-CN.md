...
- `inbox/` 是提交原始文件和确定性 Defuddle 实时 URL 捕获文件的入口。
- `raw/` 是生命周期来源工件的严格状态分区，分为 `digested`, `needs-review`, `ignored`, 和 `unsupported`。
- 来源审查门被设置在来源提取**之后**、wiki 更新**之前**。
- 被接受的来源会在 `wiki/sources/` 目录下生成内容丰富的来源卡片。
- 基于讨论得出的知识会与基于来源材料得出的知识分开独立记录。
- 工作流采用**文本优先 (text-first)**。提供方返回的附件可以保存在 `source.md` 旁，但 OCR、解释以及由图片或音频生成知识仍需显式策略和批准。
