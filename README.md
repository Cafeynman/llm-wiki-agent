...
Default source extraction preferences are recorded in `PROJECT.md`. During first project-context confirmation, the agent asks whether to configure MinerU and whether to prefer MinerU when it is available; choosing that preference records MinerU as the document default.
For service-backed providers such as MinerU precision extraction, MinerU private deployments, or OCR backends, copy `.env.example` to `.env` in the initialized working root and fill only the required variables such as `MINERU_TOKEN`. The real `.env` file is ignored by Git; agents should load it with `uv run --env-file .env` when running provider commands.
